...
- If the same test failure recurs after **2 fix attempts**, stop iterating and report the failure to the user with:
  - The exact error message and traceback
  - Whether the failure is a test code issue, source code issue, or server-side issue
  - A clear recommendation on next steps
- If the failure requires server-side changes, report this to the user immediately without further retries
