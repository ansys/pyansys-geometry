...
def test_create_circle():
    ...
    radius, origin = (1 * UNITS.m).magnitude, Point3D([0, 0, 0], UNITS.m)
...
def test_create_ellipse():
    ...
    # Draw a circle in previous sketch
    a, b, origin = (2 * UNITS.m).magnitude, (1 * UNITS.m).magnitude, Point3D([0, 0, 0], UNITS.m)
    ecc = np.sqrt(1 - (b / a) ** 2)
...
def test_create_polygon():
    ...
    radius, sides, origin = (1 * UNITS.m).magnitude, 5, Point3D([0, 0, 0], UNITS.m)
    ...
    radius, sides, origin = (1 * UNITS.m).magnitude, 4, Point3D([0, 0, 0], UNITS.m)
    ...
    with pytest.raises(
        ...
        radius, sides, origin = (1 * UNITS.m).magnitude, 2, Point3D([0, 0, 0], UNITS.m)
        sketch.draw_polygon(radius, sides, origin)

    with pytest.raises(ValueError, match="Radius must be a real positive value."):
        radius, sides, origin = (-1 * UNITS.m).magnitude, 6, Point3D([0, 0, 0], UNITS.m)
        sketch.draw_polygon(radius, sides, origin)
